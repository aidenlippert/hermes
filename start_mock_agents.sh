...
python mock_agents/translation_agent.py > logs/translation_agent.log 2>&1 &
TRANSLATION_PID=$!

echo "Starting Free Summarizer (port 8002)..."
python mock_agents/summarizer_agent.py > logs/summarizer_agent.log 2>&1 &
SUMMARIZER_PID=$!

echo "Starting Code Analyzer (port 8003)..."
python mock_agents/code_analyzer_agent.py > logs/code_analyzer_agent.log 2>&1 &
ANALYZER_PID=$!

echo "Starting LangChain Agent (port 8004)..."
python langchain_agent/agent_server.py > logs/langchain_agent.log 2>&1 &
LANGCHAIN_PID=$!
